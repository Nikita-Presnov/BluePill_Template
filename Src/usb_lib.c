...
#define ENDP_STAT_RX(num, stat)                                                                                                      \
    do                                                                                                                               \
    {                                                                                                                                \
        USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EPTX_STAT)) | USB_EP_CTR_RX | USB_EP_CTR_TX) ^ stat; \
    } while (0)
#define ENDP_STAT_TX(num, stat)                                                                                                      \
    do                                                                                                                               \
    {                                                                                                                                \
        USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EPRX_STAT)) | USB_EP_CTR_RX | USB_EP_CTR_TX) ^ stat; \
    } while (0)
#define ENDP_DTOG_RX(num, dtog)                                                                                                     \
    do                                                                                                                              \
    {                                                                                                                               \
        USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_TX | USB_EPRX_STAT | USB_EPTX_STAT)) | USB_EP_CTR_RX | USB_EP_CTR_TX) ^ dtog; \
    } while (0)
#define ENDP_DTOG_TX(num, dtog)                                                                                                     \
    do                                                                                                                              \
    {                                                                                                                               \
        USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_RX | USB_EPRX_STAT | USB_EPTX_STAT)) | USB_EP_CTR_RX | USB_EP_CTR_TX) ^ dtog; \
    } while (0)
#define ENDP_CTR_RX_CLR(num)                                                                                                                  \
    do                                                                                                                                        \
    {                                                                                                                                         \
        USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EPRX_STAT | USB_EPTX_STAT | USB_EP_CTR_RX)) | USB_EP_CTR_TX); \
    } while (0)
#define ENDP_CTR_TX_CLR(num)                                                                                                                  \
    do                                                                                                                                        \
    {                                                                                                                                         \
        USB_EPx(num) = ((USB_EPx(num) & ~(USB_EP_DTOG_RX | USB_EP_DTOG_TX | USB_EPRX_STAT | USB_EPTX_STAT | USB_EP_CTR_TX)) | USB_EP_CTR_RX); \
    } while (0)
...
#error USB_PULLUP undefined
    ...
    for (uint8_t i = 0; i < STM32ENDPOINTS; i++)
        epfunc_in[i] = epfunc_out[i] = endp_callback_default;
    for (uint32_t ctr = 0; ctr < 100000; ++ctr)
        asm volatile("nop"); // wait >1ms
    USB->CNTR = 0;
    USB->BTABLE = 0;
...
static void ep0_out(uint8_t epnum)
{
    ...
    if (setup)
    {
        ...
        { // предыдущий пакет=IN, этот точно будет запросом
            usb_ep_read(0, (uint16_t *)&setup_packet);
            bytesread = 0;
            rxcount = 0;
        }
        else
        { // предыдущий пакет=OUT, этот будет либо данными к нему, либо новым запросом
    ...
    if (setup_packet.bmRequestType & 0x80)
    {
        //---IN---
        if (!setup)
            return; // какой-то костыль. Иначе вызывается два раза
...
void USB_LP_IRQHandler()
{
    if (USB->ISTR & USB_ISTR_CTR)
    {
        do
        {
            ...
            if (USB_EPx(epnum) & USB_EP_CTR_RX)
            { // OUT
